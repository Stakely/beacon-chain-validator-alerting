...
const checkBeaconchainData = async () => {
  ...
  for (const savedValidatorsChunk of savedValidatorsChunks) {
    ...
    try {
      ...
      console.error('Beaconchain API error. URL', beaconchainUrl, 'response', JSON.stringify(res), 'Error:', error)
...
const checkAttestations = async () => {
  ...
  for (const savedValidatorsChunk of savedValidatorsChunks) {
    ...
    for (const validatorData of beaconchainData) {
      ...
      if (validatorData.epoch <= lastEpoch && validatorData.status !== 1 && validatorData.epoch > savedValidatorData.last_epoch_checked) {
        if (!aggregatedMissedAttestations[savedValidatorData.vc_name]) {
          aggregatedMissedAttestations[savedValidatorData.vc_name] = []
        }
        // Save the missed attestations to send the data aggregated by hostname
        aggregatedMissedAttestations[savedValidatorData.vc_name].push({
  ...
  for (const vcName in aggregatedMissedAttestations) {
    let text = `**Total attestations:** ${aggregatedMissedAttestations[vcName].length}`
    let attestationsCount = 0
    for (const missedAttestation of aggregatedMissedAttestations[vcName]) {
